import React from 'react'

const Point1 = () => {
  return (
    <div className=' w-full h-screen bg-white overflow-hidden border-b-1  border-black/10'>
      
      <div className="flex gap-70 p-10 text-black home-content shrink-0 ">

        <h1 className="heading text-[4vw]  font-rethink font-semibold"> <span className='text-orange-500'>01</span> Point 1</h1>

        <div className="text w-110 font-semibold text-black/60 text-[17px]">
           <span >
            Lorem ipsum, dolor sit amet consectetur adipisicing elit. Sequi ipsam velit animi consequatur maxime modi molestias totam, saepe praesentium recusandae odit 
            <br />
            <br />
             restores confidence in financial numbers.
            <br /> <br />
            <img src="/mountain.jpg" className='' alt="" />
            <br /><br />est deserunt nisi assumenda esse. Nobis provident quidem expedita corrupti reprehenderit quaerat vel assumenda tempore molestiae alias nulla, est eaque vero ipsam obcaecati saepe. In excepturi officiis officia.
           </span>
        </div>
      </div>
    </div>
  )
}

export default Point1